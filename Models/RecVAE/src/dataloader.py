...
class Preprocess:
    ...
    def load_data_from_file(self, args):
        ...
        return raw_data, unique_uid, tr_users, vd_users, te_users, n_users, unique_uid_before_shuffling
    ...
    def data_split(self, args, raw_data, unique_uid, tr_users, vd_users, te_users, unique_uid_before_shuffling):
        ...
        inference_data = self.numerize(raw_data, profile2id, show2id)
        inference_data.to_csv(os.path.join(pro_dir, 'inference.csv'), index=False)
...
class Dataloader():
    ...
    def _load_train_data(self, datatype='train'):
        path = os.path.join(self.pro_dir, '{}.csv'.format(datatype))
